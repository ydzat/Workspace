...
public:
	                   ...
	                   CGVaryings& out) const
	{
        ...
        float NdotL = std::max(CGMath::dot(vNrm, L), 0.0f);
        ...
        if (NdotL > 0.0f)
        {
            ...
            CGVec4 E = CGMath::normalize(CGVec4(0.0f, 0.0f, 0.0f, 1.0) - vPos);
            // H is halfway vector between L and E
            CGVec4 H = CGMath::normalize(L + E);
            // specular
            float NdotH = std::max(CGMath::dot(vNrm, H), 0.0f);
        ...
        out.color = CGMath::clamp(out.color, 0.0f, 1.0f);
