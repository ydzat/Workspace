...
void CGTriangleRasterizer::rasterize(const CGVaryings& A,const CGVaryings& B,const CGVaryings& C)
{
	...
	// Raster as three points. Remove this later ....
    /*
	fragment.set(A);
	fragment.coordinates.set((int)a.x,(int)a.y);
	m_frag_ops.push_fragment(fragment);
	fragment.set(B);
	fragment.coordinates.set((int)b.x,(int)b.y);
	m_frag_ops.push_fragment(fragment);
	fragment.set(C);
	fragment.coordinates.set((int)c.x,(int)c.y);
	m_frag_ops.push_fragment(fragment);
    */
